...
| 19 | Harness Engineering | An agent left to work in an unstructured environment produces results that can not be verified or reproduced. | Engineer the harness around the model: a scoped workspace, a curated set of tools, and an automated test loop that gives the agent feedback on every iteration. | Any agentic coding or task-execution loop where the agent needs to check its own work before returning a result. | [examples/19_harness_engineering](examples/19_harness_engineering) |
